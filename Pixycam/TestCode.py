	...
	for index in range (0, count):
		...
		if currentArea > 50:
			...
			if blocks[index].signature==2:
				maxAreaOrange = max(currentArea, maxAreaBlue)
				if maxAreaBlue==currentArea:
					blueIndex = index
	if blueIndex != -1:
		for index in range (0, count):
			table.putNumber("blueX", blocks[blueIndex].x)
			table.putNumber("blueY", blocks[blueIndex].y)
			table.putNumber("blueWidth", blocks[blueIndex].width)
			table.putNumber("blueHeight", blocks[blueIndex].height)
      			print '[blocks_TYPE=%d SIG=%d X=%3d Y=%3d WIDTH=%3d HEIGHT=%3d]' % (blocks[blueIndex].type, blocks[blueIndex].signature, blocks[blueIndex].x, blocks[blueIndex].y, blocks[blueIndex].width, blocks[blueIndex].height)
	if orangeIndex != -1:
		for index in range (0, count):
